...
class CRM_HelloassoPaymentProcessor_Upgrader extends CRM_Extension_Upgrader_Base {
  ...
  public function upgrade_4209(): bool {
    ...
    $json = (string) (Civi::settings()->get('helloasso_processor_auth_json') ?? '');
    if ($json === '') {
      return TRUE;
    }
    ...
    foreach ($decoded as $paymentProcessorId => $config) {
      $paymentProcessorId = (int) $paymentProcessorId;
      if (!$paymentProcessorId || !is_array($config)) {
        continue;
      }

      $oauthLink = !empty($config['oauth_link']) && is_array($config['oauth_link']) ? $config['oauth_link'] : [];
      $webhookRegistration = !empty($config['webhook_registration']) && is_array($config['webhook_registration']) ? $config['webhook_registration'] : [];
      $updatedAt = (string) ($config['updated_at'] ?? date('Y-m-d H:i:s'));

      CRM_Core_DAO::executeQuery("
        INSERT INTO civicrm_hello_asso_processor_auth (
          ...
          %1, %2, %3, %4, %5, %6, %15, %7, %8, %9, %10, %11, %12, %13, %14, %14
        )
        ON DUPLICATE KEY UPDATE
          ...
          refresh_issued_at = VALUES(refresh_issued_at),
          refresh_expires_at = VALUES(refresh_expires_at),
          linked_at = VALUES(linked_at),
          redirect_uri = VALUES(redirect_uri),
          webhook_ownership = VALUES(webhook_ownership),
          webhook_url = VALUES(webhook_url),
          webhook_signature_key = VALUES(webhook_signature_key),
          webhook_updated_at = VALUES(webhook_updated_at),
          updated_at = VALUES(updated_at)
      ", [
        1 => [$paymentProcessorId, 'Integer'],
        2 => [$this->normalizeNullableString($config['connection_mode'] ?? NULL), 'String'],
        3 => [$this->normalizeNullableString($oauthLink['organization_slug'] ?? NULL), 'String'],
        4 => [$this->normalizeNullableString($oauthLink['access_token'] ?? NULL), 'String'],
        5 => [$this->normalizeNullableString($oauthLink['refresh_token'] ?? NULL), 'String'],
        6 => $this->buildNullableIntegerParam($oauthLink['expires_at'] ?? NULL),
        7 => $this->buildNullableIntegerParam($oauthLink['refresh_expires_at'] ?? NULL),
        8 => $this->buildNullableTimestampParam($oauthLink['linked_at'] ?? NULL),
        9 => [$this->normalizeNullableString($oauthLink['redirect_uri'] ?? NULL), 'String'],
        10 => [$this->normalizeNullableString($config['webhook_ownership'] ?? (!empty($config['webhook_auto_register']) ? 'managed_by_civicrm' : NULL)), 'String'],
        11 => [$this->normalizeNullableString($webhookRegistration['url'] ?? NULL), 'String'],
        12 => [$this->normalizeNullableString($webhookRegistration['signatureKey'] ?? NULL), 'String'],
        13 => $this->buildNullableTimestampParam($webhookRegistration['updated_at'] ?? NULL),
        14 => $this->buildNullableTimestampParam($updatedAt),
        15 => $this->buildNullableIntegerParam($oauthLink['refresh_issued_at'] ?? NULL),
      ]);
    }
